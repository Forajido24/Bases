...
<head>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                window.location.href =
                    'productos_lista.php'; // Redirigir a la lista de productos después de guardar
            },
            error: function() {
                $('#mensaje').show().html('Error al guardar el producto');
            }
        });
    }
    ...
    function Alerta() {
        ...
        if (nombre && descripcion && costo && foto.files.length > 0) {
            $('#mensaje').show().html('Campos llenos');
            setTimeout(function() {
                $('#mensaje').html('');
                $('#mensaje').hide();
            }, 5000);
            ...
            <td>
                <form name="forma01" method="post" action="#" onsubmit="return false;" enctype="multipart/form-data">
                    ...
                    <input type="text" name="descripcion" id="descripcion"
                        placeholder="Escribe la descripción del producto"><br>
